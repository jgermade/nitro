...
_.extend(nitro, {
  ...
  symlink: function (_path, target, type) {
    var dirpath = joinPaths.root( _path.replace(/\/[^\/]+$/, '') );
    if( !dir.exists(dirpath) ) {
      dir.create(dirpath);
    }
    fs.symlinkSync( target, joinPaths.root(_path), type);
    return nitro;
  },
  ...
  import: function (dirpath, filter) {
    ...
    dir(dirpath).load( filter || ( filter === false ? '*.js' : '{,**/}*.js' ) )
      .each(function (f) {
        var fullpath = joinPaths.root( f.cwd, f.path );
  ...
  parseYAML: function (yaml, options) {
    options = options || {};
    return YAML.safeLoad(yaml, {
      ...
      json: options.json,
      onWarning: function () {
        console.log.apply(console, ['YAML error'].concat(arguments));
        if( options.onWarning instanceof Function ) options.onWarning.apply(this, arguments);
      }
    });
  },
  ...
  run: function (taskList) {
    ...
    return tasks.run( taskList || [].slice.call(process.argv, 2), function () {
      process.exit(0);
    });
  },
  server: server
