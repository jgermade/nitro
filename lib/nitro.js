...
_.extend(nitro, {
  ...
  import: function (dirpath, filter) {
    ...
    require('glob').sync( require('path').join(dirpath, filter || ( filter === false ? '*.js' : '{,**/}*.js' ) ) )
  ...
  libs2html: function (filter, options) {
    ...
    return files.reduce(function (prev, filepath) {
      ...
      if( /\.js$/.test(filepath) ) {
        ...
        return prev + '\n\t<link type="text/css" rel="stylesheet" href="' + filepath + ( options.version ? ( '?v=' + options.version ) : '' ) + '\" />';
      }
      return prev;
    }, '');
  },
