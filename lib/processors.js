...
function processorWrapper (processor) {
  return function (list, options) {
    var result, cwd = process.cwd();
    options = options || {};

    // if( options.cwd || list.cwd ) process.chdir( joinPaths.root(options.cwd || list.cwd) );
    // else process.chdir(process_cwd);

    result = processor.call( list, list, _.extend(list.options.sourceMap ? { sourceMap: list.options.sourceMap } : {}, options) );
