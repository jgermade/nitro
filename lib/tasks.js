...
function tasks () {
  if( arguments.length > 1 ) registerTask.apply(this, arguments);
  else if( arguments.length === 1 ) tasks.run.apply(this, arguments);
}

tasks.register = registerTask;

function runTask (task, target) {
  ...
  if( typeof task === 'string' ) {
    return (function (taskName, target) {
      ...
      return runTask(tasksCache[taskName], target ).then(function (result) {
        console.log('\n' + taskName + (target ? color.cyan(':' + target) : '') + ' ' + color.yellow( ( Date.now() - startTime ) + 'ms' ) + '\n' );
        return result;
      });
  ...
  if( task instanceof Array ) {
    return Parole.all(task.map(function (task) {
  ...
  if( typeof handlers === 'object' ) {
    ...
    return runTask(handlers.default || handlers[target], target);
  }

}

function runTasksSequence (tasks) {
