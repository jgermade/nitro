...
function loadFile (File, filepath, cwd) {
  ...
  src = src.replace(/\n?\/\/# +sourceMappingURL=(.*?)\s*$/, function (mapPatter, mapFile) {
    ...
    // map = file.read( path.join(cwd, mapFile ) );
    return '';
  });
...
Files.prototype.join = function (filter, filepath, options) {
  ...
  var sourceMap = ( options.sourceMap !== undefined ? options.sourceMap : list.options.sourceMap );

  if( sourceMap || this.every(function (f) { return f.map; }) ) {
