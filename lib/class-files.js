...
Files.prototype.join = function (filter, filepath, options) {
  ...
  if( options.sourceMap ) {
    var Concat = nitro.require('concat-with-sourcemaps'),
        concat = new Concat(true, 'all.js', '\n');

    this.each(function (f) {
      concat.add( f.getPath(), f.getSrc(), f.getMap() );
    });

    return new Files( new files.File( concat.content, file.parsePath(filepath) ) );
  }

  return new Files( new files.File( this.each(function (f) {
    return f.gerSrc();
  }, []).join('\n// ' + f.getPath() + '\n\n'), file.parsePath(filepath) ) );
};
