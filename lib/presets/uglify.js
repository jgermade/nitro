...
module.exports = function (nitro) {
  ...
  var uglify = nitro.require('uglify-js'),
      ...
      getRatioLog = function (ratio) {
        ...
        if( compression > 0.8 ) ratio_color = 'yellow';
        else if( compression < 0.8 ) ratio_color = 'green';
  ...
  return function (list, options) {
    ...
    list.forEach(function (f) {
      ...
      try{
        ...
        console.log( getRatioLog(result.code.length/f.src.length), color.magenta(f.src.length) + color.grey(' => ') + color.magenta(result.code.length) + '\n' );
