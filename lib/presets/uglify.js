...
module.exports = function (nitro) {
  ...
  return function (list, options) {
    ...
    list.forEach(function (f) {
      ...
      try{
        console.log('uglifing', color.yellow(f.path), f.src.split('\n').length );
        result = uglify.minify(o);
        console.log('uglified', f.path, typeof result, Object.keys(result) );
        ...
        console.log( '\nfile: ' + color.greyBright( f.cwd + ( /\/$/.test(f.cwd) ? '' : '/' ) ) + color.yellow(err.filename) );
