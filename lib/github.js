
module.exports = function (nitro) {

  var remotes,
      getRemotes = function () {

        if( remotes ) {
          return remotes;
        }

        var remoteLines = ('' + nitro.exec('git remote -v')).split('\n').filter(function (line) {
          return line.length;
        }),
        remotes = {};

        remoteLines.forEach(function (line) {
          var matched = line.match(/(\w+)\sgit@(\w+\.\w+):(.*?)\/(.*?)\.git \((.*?)\)$/);

          if( matched ) {
            remotes[matched[1] + '-' + matched[5] ] = {
              line: matched[0],
              hostname: matched[2],
              owner: matched[3],
              repo: matched[4]
            };
          } else {
            matched = line.match(/(\w+)\shttps:\/\/(\w+\.\w+)\/(.*?)\/(.*?)\.git \((.*?)\)$/);

            if( matched ) {
              remotes[matched[1] + '-' + matched[5] ] = {
                line: matched[0],
                hostname: matched[2],
                owner: matched[3],
                repo: matched[4]
              };
            }
          }
        });

        if( !remotes['origin-fetch'] ) {
          console.warn('origin fetch missing');
        }

        if( !remotes['origin-push'] ) {
          console.warn('origin push missing');
        }

        var remote = remotes['origin-fetch'];

        remotes.default = remote;
        remotes.isGithub = remote.hostname === 'github.com';

        return remotes;
      },
      parseVersion = function (tag_name) {
        var matches = tag_name.match(/^v(.*?)\.(.*?)\.(.*?)$/);

        return {
          major: Number(matches[1]),
          minor: Number(matches[2]),
          build: Number(matches[3])
        };
      }

  return {
    release: function (releaseName) {
      var https = require('https'),
          url = require('url'),
          _ = nitro.tools,
          remotes = getRemotes(),
          remote = remotes.default;

      if( !remotes.isGithub ) {
        throw new Error('origin is not github');
      }

      var releasesUrl = url.parse(`https://api.github.com/repos/${remote.owner}/${remote.repo}/releases`);
      var releasesUrl = `https://api.github.com/repos/${remote.owner}/${remote.repo}/releases`;

      console.log('releasesUrl', releasesUrl);

      var fetch = nitro.require('node-fetch');

      // fetching latest release
      fetch(releasesUrl, {
          headers: {
            'Accept': 'application/vnd.github.v3+json',
            'User-Agent': `release-${remote.owner}-${remote.repo}`,
            'Authorization': 'token 9b5dfa9d1a17e53e934010563f275d17a5bd8c09'
          }
        })
        .then(function(res) {
          if( res.status >= 200 && res.status < 300 ) {
            return res.json();
          }

          return null;
        }, function () {
          console.error(res);
          process.exit(2);
        })
        .then(function(releases) {
          var v = (releases && releases.length) ? releases.reduce(function (prev, release) {
            var version = parseVersion(release.tag_name);

            if( prev ) {
              if( prev.major > version.major ) {
                return prev;
              } else if( prev.major < version.major ) {
                return version;
              } else if( prev.minor > version.minor ) {
                return prev;
              } else if( prev.minor < version.minor ) {
                return version;
              } else if( prev.build > version.build ) {
                return prev;
              }
              return version;
            }
            return version;
          }, null) : { major: 0, minor: 0, build: 0 };

          return v;
        })

      .then(function (v) {
        var version;
        console.log('version', v);

        if( releaseName === 'build' || releaseName === undefined ) {
          v.build = v.build + 1;
        } else if( releaseName === 'minor' ) {
          v.minor = v.minor + 1;
        } else if( releaseName === 'major' ) {
          v.major = v.major + 1;
        } else {
          version = releaseName;
        }

        if( !version ) {
          version = `v${v.major}.${v.minor}.${v.build}`;
        }
        console.log('version', v, version);

        return fetch(releasesUrl, {
            method: 'POST',
            body: JSON.stringify({
              "tag_name": version,
              "target_commitish": "master",
              "name": version,
              "body": ` Release ${version}`,
              "draft": false,
              "prerelease": false
            }),
            headers: {
              'Accept': 'application/vnd.github.v3+json',
              'Content-Type': 'application/json',
              'User-Agent': `release-${remote.owner}-${remote.repo}`,
              'Authorization': 'token 9b5dfa9d1a17e53e934010563f275d17a5bd8c09'
            }
          }).then(function(res) {
            if( res.status >= 200 && res.status < 300 ) {
              console.log(res.ok);
              console.log(res.status);
              console.log(res.statusText);
              console.log(res.headers.raw());
              return res.json();
            }
            console.error(res);
            throw res;
          });
      }, function () {
        console.error(res);
        process.exit(2);
      })

      .then(function (release) {
        console.log('release', release);

        var mime = nitro.require('mime');

        console.log('mime', mime.lookup('test.js') );
        console.log(`https://uploads.github.com/repos/${remote.owner}/${remote.repo}/releases/${release.id}/assets`);

        return fetch(`https://uploads.github.com/repos/${remote.owner}/${remote.repo}/releases/${release.id}/assets`, {
            method: 'POST',
            body: fs.readFile('test.js'),
            headers: {
              'Accept': 'application/vnd.github.v3+json',
              'Content-Type': mime.lookup('test.js'),
              'User-Agent': `release-${remote.owner}-${remote.repo}`,
              'Authorization': 'token 9b5dfa9d1a17e53e934010563f275d17a5bd8c09'
            }
          }).then(function(res) {
            if( res.status >= 200 && res.status < 300 ) {
              console.log(res.ok);
              console.log(res.status);
              console.log(res.statusText);
              console.log(res.headers.raw());
              return res.json().then(function (release) {
                console.log('release', release);
              });
            }
            console.error(res);
            throw res;
          }, function () {
            console.error(res);
            process.exit(2);
          });
      });
    }
  };

  console.log( remoteLines, remotes );

};
