...
module.exports = function (nitro) {
  ...
  return {
    release: function (releaseName, release) {
      release = release || {};
      ...
      var releasesUrl = url.parse(`https://api.github.com/repos/${remote.owner}/${remote.repo}/releases`);
      ...
      .then(function (r) {
        ...
        //     body = nitro.file.read('test.js');

        // console.log('mime', mime.lookup('test.js'), body );
        // console.log(`https://uploads.github.com/repos/${remote.owner}/${remote.repo}/releases/${r.id}/assets?name=test.js`);

        var attachments = typeof release.attach === 'string' ? [release.attach] : release.attach;

        if( attachments ) {
          return nitro.require('q-promise').all(
            nitro.load(attachments).each(function (file) {
        ...
        return fetch(`https://uploads.github.com/repos/${remote.owner}/${remote.repo}/releases/${r.id}/assets?name=test.js`, {
            method: 'POST',
            body: body,
            headers: {
              'Accept': 'application/vnd.github.v3+json',
              'Content-Type': mime.lookup('test.js'),
              'User-Agent': `release-${remote.owner}-${remote.repo}`,
              'Authorization': `token ${process.env.GITHUB_TOKEN}`
            }
          }).then(function(res) {
