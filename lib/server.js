...
function runServer (rootpath, options) {
  ...
  var documentRoot = /^\//.test(options.root) ? options.root : path.join(cwd, options.root);
  var server = http.createServer(function(request, response) {
    ...
    tryFilesFn( basePath, uri.replace(/\/$/, ''), ['$uri'].concat( typeof options.tryFiles === 'string' ? options.tryFiles.split(/\s+/) : (options.tryFiles && options.tryFiles.slice() || ['$uri.html']) ) ).then(function (filepath) {
      ...
      if( options.log ) {
        ...
        console.log('Root directory is: '.yellow + documentRoot.green + '\n' );
      }
      ...
      if( options.livereload ) {
        ...
        livereload( runServer(__dirname + '/livereload', { port: livereloadOptions.port || 12345 }), watchDirs.map(function (dir) {
