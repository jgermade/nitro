...
_.extend(dir, {
  ...
  copy: function copyFiles (dirpath, globSrc, dest, options) {
    ...
    dirExpand(globSrc, _.extend({}, options.cwd ? { cwd: options.cwd } : {} ) ).forEach(function (filepath) {
      var srcPath = joinPaths.root( options.cwd || '.', filepath ),
          destPath = joinPaths.root(dest || '.', filepath);
  ...
  load: function (dirpath, pattern, options) {
    return new Files( dirExpand(pattern, _.extend(options || {}, { cwd: joinPaths.root(dirpath) }) ), { cwd: dirpath });
  },
