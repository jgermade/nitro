...
function Watcher (dirPath, options, handler) {
  ...
  if( options.livereload ) {
    // var wss = server(__dirname + '/livereload', { port: options.livereload, log: false });
    var wss = server(__dirname + '/livereload', { port: options.livereload });
    var ws = new autoRequire('ws').Server({ server: wss });
    ws.on('connection', function(w) {

      livereload = function (data) {
  ...
  // }, function (eventName, filepath) {
    ...
    if( livereload ) livereload({ filepath: filepath });
  });
