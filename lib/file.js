...
var file = {
  ...
  read: function () {
    return fs.readFileSync( joinPaths.root.apply(this, arguments), { encoding: 'utf8' });
  },
  readJSON: function () {
  ...
  write: function (paths, content, options, done) {
    ...
    if( done instanceof Function ) {
      ...
      return fs.writeFile( joinPaths.root(dest) , content, _.extend({ encoding: 'utf8' }, options || {}, done) );
    } else {
      if( filepath ) mkdirp.sync(filepath);
