...
module.exports = function (nitro) {
  ...
  function expandDependencies (dependencies, found, map, inCascade, filter, options) {
    ...
    dependencies.each(function (dependence, version) {
      ...
      if( inCascade && pkg.dependencies ) {
        ...
        found = found.concat(
          expandDependencies( new Package(pkg, meta).dependencies(), [], map, inCascade, filter, options )
  ...
  function inDependenceList (matcher) {
  ...
  Dependencies.prototype.expand = function (inCascade, options) {
    ...
    var filter = options.filter || (options.only && function (dependenceName, version) {
      return options.only.some(inDependenceList);
    }) || (options.exclude && function (dependenceName, version) {
      return !options.exclude.some(inDependenceList);
    }) || function (dependenceName, version) { return true };

    return expandDependencies(this, [], {}, inCascade, filter, options);
  };
