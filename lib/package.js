...
module.exports = function (nitro) {
  ...
  function pkgFileName (pkgName, cwd) {
    ...
    return ( pkgFiles[pkgName] || [] ).find(function (filename) {
      return nitro.file.exists( joinPaths(cwd, filename) );
    });
  }
  ...
  function readPkg (pkgName, cwd) {
    ...
    if( !filename ) {
      throw new Error(filename + ' not found in ' + joinPaths(cwd || '.') + ' for pkg: ' + pkgName);
    }

    return nitro.file.readJSON( joinPaths(cwd || '.', filename) );
  }
  ...
  function expandDependencies (dependencies, options, found, map) {
    ...
    dependencies.each(function (dependence, version) {
      ...
      var pkg = readPkg(meta.name, joinPaths(meta.dependenciesdir, dependence) ),
          dependencedir = joinPaths( meta.dependenciesdir, dependence ),
      ...
      if( pkgMain ) {
        found = found.concat( nitro.dir( dependencedir ).expand(pkgMain).map(function (filename) {
          return joinPaths(dependence, filename);
        }) );
      }

      if( options.inCascade && pkg.dependencies ) {
        var cwd;
        if( nitro.dir.exists( joinPaths(dependencedir, meta.dependenciesdir) ) ) {
          cwd = process.cwd();
          process.chdir( joinPaths.root(dependencedir) );
        }

        found = found.concat(
          expandDependencies( new Package(pkg, meta).dependencies(), options, [], map )
            .map(function (filepath) {
              return cwd ? joinPaths(dependencedir, filepath) : filepath;
            })
        );
  ...
  Dependencies.prototype.copy = function (dest, options) {
    ...
    expandDependencies( this, options ).forEach(function (filepath) {
      var filepathFull = joinPaths(dependenciesdir, filepath);

      if( nitro.isFile(filepathFull) ) {
        nitro.file.copy( filepathFull, joinPaths(dest, options.expand ? filepath : nitro.file.parsePath(filepath).filename ) );
      } else if( nitro.isDir(filepathFull) ) {
        nitro.dir.copy( filepathFull, joinPaths( dest, filepath ) );
      }
    });
    return this;
  };
